...
class game:
    ...
    def get_state(self):
        ...
        for i in range(head[1]+1):
            if [head[0],head[1]-i-1] in body or head[1]-i-1 <0:
                state[4] = i
                break
        for i in range(border-head[1]+1):
            if [head[0],head[1]+i+1] in body or head[1]+i+1 >= border:
                state[5] = i
                break
        for i in range(head[0]+1):
            if [head[0]-i-1,head[1]] in body or head[0]-i-1 <0:
                state[6] = i
                break
        for i in range(border-head[0]+1):
            if [head[0]+i+1,head[1]] in body or head[0]+i+1 >= border:
                state[7] = i
                break
    
        #Diagonal Sensor
        for i in range(border-head[0]+1):
            if [head[0]-i-1,head[1]-i-1] in body or head[0]-i-1 <0 or head[1]-i-1 <0:
                state[8] = i
                break
        for i in range(border-head[0]+1):
            if [head[0]+i+1,head[1]-i-1] in body or head[0]+i+1 >= border or head[1]-i-1 <0:
                state[9] = i
                break
        for i in range(border-head[0]+1):
            if [head[0]-i-1,head[1]+i+1] in body or head[0]-i-1 <0 or head[1]+i+1 >= border:
                state[10] = i
                break
        for i in range(border-head[0]+1):
            if [head[0]+i+1,head[1]+i+1] in body or head[0]+i+1 >= border or head[1]+i+1 >= border:
                state[11] = i
                break
        state[12] = min(8,np.absolute(head[0]-food[0])+np.absolute(head[1]-food[1]))
        return state